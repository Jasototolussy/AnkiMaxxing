...
def test_maximize_anki_calls_show_window():
    with patch("window.win32gui.EnumWindows", side_effect=lambda cb, lst: _enum_found(12345, lst)), \
         ...
         patch("window.win32gui.GetForegroundWindow", return_value=99999), \
         patch("window.win32process.GetWindowThreadProcessId", return_value=(1, 2)), \
         patch("window.win32api.GetCurrentThreadId", return_value=1), \
         patch("window.win32process.AttachThreadInput"), \
         patch("window.win32gui.SetForegroundWindow") as mock_fg:
        maximize_anki()
